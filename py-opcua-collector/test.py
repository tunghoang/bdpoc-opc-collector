...
async def main():
    ...
    async with Client(url=url) as client:
        ...
        _logger.info("node: %r", (await client.nodes.root.get_child(["0:Objects", "1:PHDServer", "1:HT_ZI_2180A.PV"])))
        ...
        objnode = client.nodes.objects
        ochilds = await objnode.get_children()
        _logger.info("nn: %r", ochilds[1])
        _logger.info("node: %r", (await ochilds[1].get_child(["1:HT_ZI_2180A.PV"])))
        ochilds = await ochilds[1].get_children()
        _logger.info("childs: %r", (await ochilds[0].read_browse_name()))
        val = await ochilds[0].read_value()
        _logger.info("valu: %r", val)
